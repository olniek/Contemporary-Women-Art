...
export function openArtistDetailPanel(artist, options = {}) {
  ...
  if (options.onAskFollowUp) {
    ...
    askBtn.type = "button";
    const topicBit = options.topicLabel
      ? ` and how their work relates to ${options.topicLabel}`
      : "";
    askBtn.addEventListener("click", () => {
      dialog.close();
      options.onAskFollowUp(`Tell me more about ${artist.name} — their practice${topicBit}.`);
    });
